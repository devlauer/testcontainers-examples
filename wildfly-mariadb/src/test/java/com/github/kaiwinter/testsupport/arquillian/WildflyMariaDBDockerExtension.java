...
public final class WildflyMariaDBDockerExtension implements LoadableExtension {
	...
	public static final class LoadContainerConfiguration {
		 ...
		 * startup time.
		 * 
		 * @param paramRegistry      contains containers defined in arquillian.xml
		 * @param paramServiceLoader
		 */
		public void registerInstance(@Observes ContainerRegistry paramRegistry, ServiceLoader paramServiceLoader) {
			...
			MariaDBContainer<?> mariaDBContainer = new MariaDBContainer<>(mariaDBImageName)
					.withNetwork(mariaDBAppserverNetwork).withNetworkAliases(MARIA_DB_NETWORK_HOSTNAME)
					.withExposedPorts(MARIADB_PORT).withDatabaseName("test").withPassword("admin")
					.withUsername("admin");
					...
					new ImageFromDockerfile()
							.withDockerfileFromBuilder(builder -> builder.from("quay.io/wildfly/wildfly:26.1.2.Final")
									...
									.cmd("/opt/jboss/wildfly/bin/standalone.sh", "-b", "0.0.0.0", "-bmanagement",
											"0.0.0.0")
									.build()))
					.withExposedPorts(WILDFLY_MANAGEMENT_PORT, WILDFLY_HTTP_PORT).withNetwork(mariaDBAppserverNetwork)
					.withStartupTimeout(Duration.ofSeconds(30));
			wildflyContainer.start();
			addDatasourceToWildflyContainer(wildflyContainer);

			configureArquillianForRemoteWildfly(wildflyContainer, paramRegistry);

			setupDb(mariaDBContainer);
		}

		private void addDatasourceToWildflyContainer(GenericContainer<?> wildflyContainer) {
			...
			File driverFile = Maven.resolver().loadPomFromFile("pom.xml")
					.resolve("org.mariadb.jdbc:mariadb-java-client").withoutTransitivity().asSingleFile(); //
			try {
			...
			request.get("connection-url").set(
					"jdbc:mariadb://"+MARIA_DB_NETWORK_HOSTNAME+"/test?autoReconnect=true&useUnicode=yes&characterEncoding=UTF-8&useSSL=false&connectTimeout=1000&allowMasterDownConnections=true&maxReconnects=2");
